...
Shader "CW3/VFXLavaLakeTesselation" {
    Properties {
        ...
        _TesselationAmount ("TesselationAmount", Range(0, 50)) = 0
        ...
        Pass {
            ...
            VertexOutput vert (VertexInput v) {
                ...
                void displacement (inout VertexInput v){
                    float2 node_1447 = (v.texcoord0*(100.0*v.vertexColor.g*float2(_UVScale.r,_UVScale.g)));
                    ...
                    float4 node_3255 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_6002, _MainTex),0.0,0));
                    float2 node_8614 = (((node_3255.b*2.0+-1.0)*_DistortionIntensity)+(_UVScale1*node_1447));
                    float4 node_7065 = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_8614, _MainTex),0.0,0));
                    float2 node_4804 = (node_9350+(float2(_ScrollingSpeed.r,_ScrollingSpeed.g)*node_7358.g));
                    float4 _node_7065_copy = tex2Dlod(_MainTex,float4(TRANSFORM_TEX(node_4804, _MainTex),0.0,0));
                    float node_6472 = (node_7065.r+(_Remap.b + ( (_node_7065_copy.g - _Remap.r) * (_Remap.a - _Remap.b) ) / (_Remap.g - _Remap.r)));
                    v.vertex.xyz += (v.normal*lerp(0.0,((1.0 - (1.0 - node_6472))-_TessMidPoint),lerp(lerp(_Displacement.r,_Displacement.g,v.vertexColor.r),_Displacement.b,v.vertexColor.b)));
                }
                float4 Tessellation(TessVertex v, TessVertex v1, TessVertex v2){
                    return UnityEdgeLengthBasedTess(v.vertex, v1.vertex, v2.vertex, _TesselationAmount);
                }
